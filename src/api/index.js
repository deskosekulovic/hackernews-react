...
export function fetchingIds(name) {
  ...
  if (
    cacheIds &&
    cacheIds[`${name} timeStamp`] &&
    Math.round((now - cacheIds[`${name} timeStamp`]) / 1000 > 59)
  ) {
    isOld = true;
  }
...
export function fetchItem(type, id) {
  ...
  if (
    cacheItems &&
    cacheItems[id] &&
    Math.round((now - cacheItems[id].timeStamp) / 1000) > 59
  ) {
    isOld = true;
  }
  ...
  if (cacheItems && cacheItems[id] && !isOld) {
    ...
    return new Promise(resolve => {
      db
        ...
        .once('value')
        .then(snapshot => {
          let item = Object.assign({}, snapshot.val(), {
            visible: true,
            timeStamp: Date.now()
          });
          resolve(item);
...
export function fetchItemsFromTypes(name, page, itemsPerPage) {
  return fetchingIds(name).then(ids =>
